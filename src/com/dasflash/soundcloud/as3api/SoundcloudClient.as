package com.dasflash.soundcloud.as3api
{
	...
	public class SoundcloudClient extends EventDispatcher
	{
		...
		{
			// Workaround: to create url request for AIR on Linux comment out the next two lines 
			// and comment in the rest of this method
			// var userAuthReq:URLRequest = new URLRequest(authURL + "?oauth_token=" + requestToken.key);
		...
		public function getAccessToken(verificationCode:String=null, externalRequestToken:OAuthToken=null):SoundcloudDelegate
		{
			...
			if (verificationCode)
				requestParams.oauth_verifier = verificationCode;
			...
			requestToken:OAuthToken=null):SoundcloudDelegate
		{
			...
			// use resource string if it is a full URL
			...
			// create delegate
			var delegate:SoundcloudDelegate = new SoundcloudDelegate(
				url,
				method,
				consumer,
				token,
